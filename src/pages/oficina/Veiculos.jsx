...
export default function Veiculos() {
  ...
  return (
    <div style={{ width: "100%" }} className="animate-fade-in page-content">
      <PageHeader
        title="Veículos"
        subtitle={`${veiculos.length} veículo${veiculos.length !== 1 ? 's' : ''} cadastrado${veiculos.length !== 1 ? 's' : ''}`}
        actions={<button className="btn-primary" onClick={openCreate}><Plus size={15} /> Novo veículo</button>}
      />

      <SearchBar value={search} onChange={setSearch} placeholder="Buscar por placa, marca, modelo ou cliente..." resultCount={filtered.length} style={{ marginBottom: 20 }} />
      ...
      <div style={{ background: 'var(--bg-800)', border: '1px solid var(--bg-500)', borderRadius: 14, overflow: 'hidden', flex: 1, minHeight: 0 }}>
        {loading ? (
          ...
          <EmptyState icon={Car} title={search ? 'Nenhum veículo encontrado' : 'Nenhum veículo cadastrado'} subtitle={search ? `Sem resultados para "${search}"` : 'Cadastre o primeiro veículo'} />
        ) : (
      ...
      {deleteId && (
        <ConfirmModal title="Excluir veículo" message="Esta ação não pode ser desfeita." onConfirm={handleDelete} onClose={() => setDeleteId(null)} />
      )}
