...
export default function Reservas() {
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    if (conflito) return showToast('Quarto já reservado neste período!', 'error')
    setSaving(true)
    const payload = {
  ...
  return (
    <div style={{ width: "100%" }} className="animate-fade-in page-content">
      <PageHeader
        title="Reservas"
        subtitle={`${reservas.length} reserva${reservas.length !== 1 ? 's' : ''} no total`}
        actions={<button className="btn-primary" onClick={openCreate}><Plus size={15} /> Nova reserva</button>}
      />
      ...
      {/* Search */}
      <SearchBar value={search} onChange={setSearch} placeholder="Buscar por hóspede, quarto..." resultCount={filtered.length} style={{ marginBottom: 20 }} />
      ...
      <div style={{ background: 'var(--bg-800)', border: '1px solid var(--bg-500)', borderRadius: 14, overflow: 'hidden', flex: 1, minHeight: 0 }}>
        {loading ? (
          ...
          <EmptyState icon={Calendar} title={search || filterStatus !== 'todos' ? 'Nenhuma reserva encontrada' : 'Nenhuma reserva ainda'} subtitle={search || filterStatus !== 'todos' ? 'Tente outros filtros' : 'Crie a primeira reserva'} />
        ) : (
      ...
      {deleteId && (
        <ConfirmModal title="Excluir reserva" message="Esta ação não pode ser desfeita." onConfirm={handleDelete} onClose={() => setDeleteId(null)} />
      )}
